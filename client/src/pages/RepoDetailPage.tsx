...
type TabKey = 'overview' | 'commits' | 'prs' | 'issues' | 'workflows' | 'releases';
...
export function RepoDetailPage() {
  ...
  useEffect(() => {
    if (!owner || !repo || activeTab === 'overview') return;
    if (activeTab === 'commits') get<Commit[]>(`/chef/github/repos/${owner}/${repo}/commits`).then(setCommits).catch(() => {});
    if (activeTab === 'prs') get<PRItem[]>(`/chef/github/repos/${owner}/${repo}/prs`).then(setPRs).catch(() => {});
    if (activeTab === 'issues') get<IssueItem[]>(`/chef/github/repos/${owner}/${repo}/issues`).then(setIssues).catch(() => {});
    if (activeTab === 'workflows') get<WorkflowRun[]>(`/chef/github/repos/${owner}/${repo}/workflows`).then(setWorkflows).catch(() => {});
    if (activeTab === 'releases') get<Release[]>(`/chef/github/repos/${owner}/${repo}/releases`).then(setReleases).catch(() => {});
  }, [activeTab, owner, repo]);

  const tabs: Array<{ key: TabKey; label: string }> = [
    { key: 'overview', label: 'OVERVIEW' },
    { key: 'commits', label: 'COMMITS' },
    { key: 'prs', label: `PRS${prs.length > 0 ? ` (${prs.length})` : ''}` },
    { key: 'issues', label: `ISSUES${issues.length > 0 ? ` (${issues.length})` : ''}` },
    { key: 'workflows', label: 'CI/CD' },
  ...
  return (
    <div className="p-3 space-y-3 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-mono text-sm text-neo-text-primary">{owner}/{repo}</span>
        {detail?.private && (
          <span className="text-[9px] font-mono px-1 border border-neo-red/30 text-neo-red/60 uppercase">PRIVATE</span>
        )}
        {detail?.url && (
          <a
            href={detail.url}
            ...
            className="ml-auto text-[10px] font-mono text-neo-text-disabled hover:text-neo-red transition-colors"
          >
            GITHUB ↗
          </a>
        )}
      ...
      {/* Tab bar */}
      <div className="flex gap-1 border-b border-neo-border/30 pb-0">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            ...
            className={`px-3 py-1.5 text-[10px] font-mono uppercase border-b-2 transition-colors ${
              activeTab === tab.key
                ? 'border-neo-red text-neo-red'
                : 'border-transparent text-neo-text-disabled hover:text-neo-text-secondary'
            }`}
          >
            {tab.label}
          </button>
      ...
      {activeTab === 'workflows' && (
        <Card title="CI/CD Workflows">
          <div className="space-y-1">
            {workflows.map((wf) => {
              const badge = conclusionBadge[wf.conclusion ?? ''] ?? 'bg-neo-bg-elevated text-neo-text-disabled border-neo-border';
              return (
                <a
                  key={wf.id}
                  href={wf.url}
